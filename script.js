const models = {
  ...
  windy: { name:'Snow radar', phase:'Live precipitation', phaseLabel:'LIVE OBSERVATIONS · INTERACTIVE WEATHER RADAR', embedded:true, url:'https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=default&metricTemp=default&metricWind=default&zoom=5&overlay=radar&product=radar&level=surface&lat=38.135&lon=-79.146&message=true', description:'An interactive Windy live weather-radar map centered on your chosen area. It displays precipitation echoes, including rain, snow, and mixed precipitation—not snow only.', action:'Pan and zoom to follow precipitation as it moves. Use local temperatures and precipitation type to judge whether the echoes over your area are likely snow.' },
...
const windyFrame = document.querySelector('#windy-frame');
...
function showModel(key) {
  ...
  const isEmbedded = model.embedded === true;
  document.body.classList.toggle('is-comparison', isComparison);
  image.hidden = isComparison || isEmbedded;
  windyFrame.hidden = !isEmbedded;
  comparisonView.hidden = !isComparison;
  singleTimeline.hidden = isComparison || isEmbedded;
  comparisonTimelines.hidden = !isComparison;
  loopControls.hidden = isEmbedded;
  document.body.classList.toggle('has-loop', !isEmbedded);
  if (isEmbedded) windyFrame.src = model.url;
  if (isComparison) {
  ...
  if (!isComparison && !isEmbedded && model.source === 'psu-nbm' && !nbmRun) {
  ...
  } else if (!isComparison && !isEmbedded) {
    playButton.disabled = false;
    renderForecastFrame();
  }
